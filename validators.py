...
def is_stt_block_limit(user_id, duration):
    ...
    audio_blocks = math.ceil(int(duration) / 15)
    if audio_blocks == 0:
        audio_blocks = 0
        return audio_blocks
    ...
    if count_all_blocks(user_id) == int:
        count_block = count_all_blocks(user_id)
        return count_block

    if count_all_blocks(user_id) == tuple:
        count_block = count_all_blocks(user_id)[0]
        return count_block
...
def is_tts_symbol_limit(user_id, text):
    ...
    if count_all_symbol(user_id) == int:
        count_symbol = count_all_symbol(user_id)
        return count_symbol

    if count_all_symbol(user_id) == tuple:
        count_symbol = count_all_symbol(user_id)[0]
        return count_symbol

    if count_all_symbol(user_id) == None:
        count_symbol = 0
        return count_symbol
        
    # Функция из БД для подсчёта всех потраченных пользователем символов
    all_symbols = count_symbol + text_symbols
